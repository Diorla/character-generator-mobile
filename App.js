// Style it, so that it will have different backgrounds

import React from "react";
import { StyleSheet, Text, View, Picker, ScrollView, Modal, Alert } from 'react-native';
import { Appbar, Divider, Surface, FAB, Card, Button, Portal, Provider } from 'react-native-paper';
...
class App extends React.Component {
  ...
  generateData=()=>{
    ...
    this.reset();
    const { height, weight, weightType, heightType } = 
      calculateHW(age, gender)
    ...
    .then(()=> {
      this.setState({
        buttock, shoulder, hasDimple, earSize,
        earShape, eyeBrowShape, eyeBrowSize, 
        eyeColour, eyeDistance, eyeElevation,
        eyeShape, faceShape, chin, doubleChin,
        forehead, skinColour, freckles, hairSize, 
        hairType, hipSize, limbThickness, weightType,
        handFeetSize, lip, neck, noseSize, heightType,
        noseShape, stomach, height, weight, hairColour,
        limbLength
      })
    })
    .then(()=> {
    ...
    return (
      <View style={{height: "100%"}}>
        ...
        <Appbar style={{backgroundColor: "teal", justifyContent: "space-around"}}>
          <Text style={{color: "white", fontWeight: "bold"}}>Character Generator</Text>
        </Appbar>
        ...
        <View style={{backgroundColor: "#A9DFBF"}}>
          ...
          <View style={{flexDirection: "row", justifyContent: "space-around"}}>
            <InputCheckbox label="Dark" isActive={this.state.isDark} onPress={()=>{
              this.setState({
                isDark: !this.state.isDark
              })
            }}/>
            <InputCheckbox label="Tan" isActive={this.state.isTan} onPress={()=>{
              this.setState({
                isTan: !this.state.isTan
              })
            }}/>
            <InputCheckbox label="Medium" isActive={this.state.isMedium} onPress={()=>{
              this.setState({
                isMedium: !this.state.isMedium
              })
            }}/>
            <InputCheckbox label="Fair" isActive={this.state.isFair} onPress={()=>{
              this.setState({
