...
const getDistFeat = () => {
  ...
  // According to wiki, it affects 1% of the population
  const vitiligo = oddCalculator([true, false], [1, 99]);
  const celeb = oddCalculator([true, false], [1, 99]);
  ...
  if (piercing) {
    return oddCalculator(["A nose piercing", "A tongue piercing"], [99, 1]);
  }
  if (gapTooth) return "Have gaps between teeth";
  if (vitiligo)
    return oddCalculator([
      "Vitiligo around both hands",
      "Vitiligo around the mouth",
      "Vitiligo on the face(eyes, nostrils and mouth)",
      "Vitiligo around both hands",
      "Vitiligo around the right hands",
      "Vitiligo around the left hands"
    ]);
  if (celeb) return "Has a striking resemblance to a celebrity";
